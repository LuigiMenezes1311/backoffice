...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [productData, currenciesData, modifierTypesData] = await Promise.all([
  ...
  const formatPriceWithModifier = (basePrice: number, modifierId: string | null, currencyId: string) => {
    ...
    const adjustedPrice = calculateAdjustedPrice(basePrice, modifierId)
    ...
    return (
      <div>
        ...
        <div className="text-xs text-muted-foreground">
          (Base: {symbol} {basePrice.toFixed(2)})
        </div>
      </div>
    )
  }
            ...
            <div className="space-y-4">
              {product.prices.length === 0 ? (
                ...
                product.prices.map((price, index) => {
                  const modifierInfo = getModifierInfo(price.modifierTypeId)
                  return (
                      ...
                      <div className="grid gap-2">
                        <div className="flex justify-between">
                          ...
                          <div className="text-right">
                            {formatPriceWithModifier(price.amount, price.modifierTypeId, price.currencyId)}
                          </div>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <div className="text-right">
                            <p className="text-sm font-medium">{modifierInfo.name}</p>
                            {modifierInfo.priceAdjustment && (
                              <p className="text-xs text-muted-foreground">
                                {modifierInfo.priceAdjustment.type === "MULTIPLIER"
                                  ? `Multiplicador: ${modifierInfo.priceAdjustment.value}x`
                                  : `Adicional: ${modifierInfo.priceAdjustment.value}`}
                              </p>
                            )}
                          </div>
                        </div>
